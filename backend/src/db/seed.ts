...
const pool = new Pool({
  connectionString: process.env.POSTGRES_URL || 'postgres://postgres:postgres@localhost:5432/interview_app'
});

const questionsData: CreateQuestionRequest[] = [
  {
  ...
  },
  {
    ...
    examples: [
      'I was assigned to a project that required using React Native, a framework I had no prior experience with. The deadline was tight - just three weeks to build a mobile app prototype. I started by identifying the core concepts I needed to learn: components, state management, navigation, and API integration. I focused on practical learning, building small features and testing them immediately. I used official documentation, tutorials, and Stack Overflow. When I encountered problems, I would search for similar issues or ask teammates with React Native experience. I also studied the codebase of a similar app our team had built previously. Within a week, I had a basic understanding and could build simple features. By the end of three weeks, I had completed a functional prototype that met all requirements. The project taught me the importance of focused, hands-on learning and leveraging available resources.',
      'When our team decided to migrate from a monolithic architecture to microservices, I was tasked with learning Go and Kubernetes in a short timeframe. I started with Go fundamentals, building small command-line tools to get comfortable with the syntax and concurrency model. Then I moved to Kubernetes, setting up a local cluster and deploying simple applications. I used interactive tutorials and followed best practices from documentation. I also pair-programmed with experienced team members to accelerate my learning. Within a month, I was productive, and within three months, I was leading the development of a critical microservice. The key was balancing theoretical understanding with practical application and knowing when to ask for help.'
    ]
  }
];
...
async function seedDatabase() {
  ...
  try {
    ...
    await pool.query('CREATE INDEX IF NOT EXISTS idx_questions_level ON questions(level)');
    await pool.query('CREATE INDEX IF NOT EXISTS idx_questions_type ON questions(type)');
    await pool.query('CREATE INDEX IF NOT EXISTS idx_questions_industry ON questions(industry)');
    ...
    for (const q of questionsData) {
      await pool.query(
        ...
        [q.question, q.level, q.type, q.industry, q.explanation, JSON.stringify(q.examples)]
      );
    }
